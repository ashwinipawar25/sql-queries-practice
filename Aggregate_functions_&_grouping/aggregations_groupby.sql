...
select is_active,count(*) as total_activeness 
from employee
group by is_active;
...
select position, count(*) from employee
group by position 
...
select name,department,position,gender ,sum(salary) 
from employee
group by name,department,position,gender;
